...
public class AuthController : BaseController
{
    ...
    [HttpPost("Login")]
    public async Task<IActionResult> Login([FromBody] Application.Features.Auth.Dtos.UserForLoginDto userForLoginDto)
    {
    ...
    public async Task<IActionResult> Register([FromBody] UserForRegisterDto<MemberForRegisterDto> memberForRegisterDto)
        {
        MemberRegisterCommand memberRegisterCommand = new() { MemberForRegisterDto = memberForRegisterDto, IpAddress = getIpAddress() };
        RegisteredResponse result = await Mediator.Send(memberRegisterCommand);
        setRefreshTokenToCookie(result.RefreshToken);
        return Created(uri: "", result.AccessToken);
    }
    [HttpPost("RegisterEmployee")]
    public async Task<IActionResult> RegisterEmployee([FromBody] UserForRegisterDto<EmployeeForRegisterDto> employeeForRegisterDto)
    {
    ...
    { // Yalnızca çerezin istek atıldığında erişmesi için ayarlanmış expire 7 gün//
        CookieOptions cookieOptions = new() { HttpOnly = true, Expires = DateTime.UtcNow.AddDays(7) };
        Response.Cookies.Append(key: "refreshToken", refreshToken.Token, cookieOptions);// Çerezi tarayıcıya ekleme
    }
}