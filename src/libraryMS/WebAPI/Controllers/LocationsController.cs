...
public class LocationsController : BaseController
{
    ...
    public async Task<IActionResult> GetList([FromQuery] PageRequest pageRequest)
    {
        ...
        GetListResponse<GetListLocationItemDto> response = await Mediator.Send(getListLocationQuery);
        return Ok(response);
    }
    ...
    {
        GetDynamicLocationQuery query = new GetDynamicLocationQuery() { Dynamic = dynamic, PageRequest = pageRequest };

        GetListResponse<GetDynamicLocationItemDto> response = await Mediator.Send(query);
