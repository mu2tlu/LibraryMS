...
public class UpdateAuthorCommand : IRequest<UpdatedAuthorResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    ...
    public string[] Roles => [Admin, Write, AuthorsOperationClaims.Update, AuthorsOperationClaims.Member, AuthorsOperationClaims.Employee];
    ...
    public class UpdateAuthorCommandHandler : IRequestHandler<UpdateAuthorCommand, UpdatedAuthorResponse>
    {
        ...
        public async Task<UpdatedAuthorResponse> Handle(UpdateAuthorCommand request, CancellationToken cancellationToken)
        {
            ...
            await _authorBusinessRules.CheckIfAuthorNameAndSurnameExists(request.Name,request.Surname); // Gerek yok // 
