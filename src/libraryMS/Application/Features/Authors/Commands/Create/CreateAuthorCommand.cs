...
public class CreateAuthorCommand : IRequest<CreatedAuthorResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    ...
    public string[] Roles => [Admin, Write, AuthorsOperationClaims.Create, AuthorsOperationClaims.Member, AuthorsOperationClaims.Employee];
    ...
    public class CreateAuthorCommandHandler : IRequestHandler<CreateAuthorCommand, CreatedAuthorResponse>
    {
        ...
        public async Task<CreatedAuthorResponse> Handle(CreateAuthorCommand request, CancellationToken cancellationToken)
        {
            ...
            await _authorBusinessRules.CheckIfAuthorNameAndSurnameExists(author.Name, author.Surname);
            Author addedAuthor = await _authorRepository.AddAsync(author);
            CreatedAuthorResponse response = _mapper.Map<CreatedAuthorResponse>(addedAuthor);
            return response;
        }
    }
}