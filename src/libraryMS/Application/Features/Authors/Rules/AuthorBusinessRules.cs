...
public class AuthorBusinessRules : BaseBusinessRules
{
    ...
    public async Task CheckIfAuthorNameAndSurnameExists(string name,string surname)
    {
        Author? author = await _authorRepository.GetAsync
            (a=>a.Name.Equals(name) && a.Surname.Equals(surname));

        if (author is not null)
        {
           await throwBusinessException(AuthorsBusinessMessages.AuthorNameAndSurnameAlreadyExists);
        }
    }
