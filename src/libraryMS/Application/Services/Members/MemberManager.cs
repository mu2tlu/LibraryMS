...
public class MemberManager : IMemberService
{
    ...
    public async Task UpdateMemberDebtByIdAsync(Guid memberId,double fineAmount)
    {
        Member? member = await _memberRepository.GetAsync(m => m.Id == memberId);
        await _memberBusinessRules.MemberShouldExistWhenSelected(member);
        member!.TotalDebt += fineAmount;
